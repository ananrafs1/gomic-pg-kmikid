...
func (m *komikid) GetRoot(title string, Page, Quantity int) ([]string, error) {
	...
	c.OnHTML("ul.chapters", func(e *colly.HTMLElement) {
		e.ForEach(".chapter-title-rtl > a", func(index int, ch *colly.HTMLElement) {
	...
	err := c.Visit(fmt.Sprintf(`https://%s/manga/%s/`, m.GetBaseURL(), title))
	if err != nil {
		return nil, err
	}
...
func (m *komikid) ParseData(links string) ([]Resource, error) {
	...
	contentGrabber.OnHTML("div#all", func(el *colly.HTMLElement) {
		imgs := []string{}
